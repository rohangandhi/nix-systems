...
4. Generate your login hash with `nix-shell -p mkpasswd --run 'mkpasswd -m yescrypt'`. Replace `!` in `zion/os/users.nix`. Do not publish your real hash. Change `time.timeZone` in `zion/system.nix` if wanted.
